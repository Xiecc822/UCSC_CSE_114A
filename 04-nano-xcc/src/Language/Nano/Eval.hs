...
eval env (EBin op e1 e2) =
  let v1 = eval env e1
      v2 = eval env e2
  in evalOp op v1 v2
eval env (EIf cond et ef) =
  case eval env cond of
    VBool True  -> eval env et
    VBool False -> eval env ef
    _ -> throw (Error "type error: if condition is not a boolean")
eval env (ELet x e1 e2) =
  let recEnv = extendEnv x v env
        where v = eval recEnv e1
  in eval recEnv e2
...
eval env (EApp e1 e2) =
  let fun = eval env e1
      arg = eval env e2
  in case fun of
       VClos env' x body -> eval (extendEnv x arg env') body
       VPrim f           -> f arg
...
-- Boolean equality/inequality (works on any pair of values)
evalOp Eq  v1 v2 = VBool (v1 == v2)
evalOp Ne  v1 v2 = VBool (v1 /= v2)
-- Order comparisons (only for integers)
...
evalOp op _ _ = throw (Error ("type error: binop " ++ show op))
...
headFun :: Value -> Value
headFun (VPair h _) = h
headFun _ = throw (Error "head: type error or empty list")

tailFun :: Value -> Value
tailFun (VPair _ t) = t
tailFun _ = throw (Error "tail: type error or empty list")
