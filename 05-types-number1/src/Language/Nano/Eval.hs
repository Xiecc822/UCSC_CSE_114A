...
eval env (EIf c t e) =
  case eval env c of
    ...
    _ -> throw (Error "type error: if condition must be a boolean")
...
eval env (EApp e1 e2)    =
  ...
  in case v1 of
       ...
       VPrim f           -> f v2
       _ -> throw (Error "type error: not a function")
...
evalOp Div (VInt i1) (VInt i2)
  | i2 == 0                    = throw (Error "division by zero")
  | otherwise                  = VInt (i1 `div` i2)
