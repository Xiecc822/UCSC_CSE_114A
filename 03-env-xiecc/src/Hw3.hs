...
varExprListEval env (Var s)       = lookupInEnv s env
...
instance Eq VarExpr where
  ...
  (==) e1 e2 = evalNoVars e1 == evalNoVars e2
    where
      evalNoVars :: VarExpr -> Int
      evalNoVars (NumVE n)       = n
      evalNoVars (Var _)         = 0
      evalNoVars (PlusVE a b)    = evalNoVars a + evalNoVars b
      evalNoVars (MinusVE a b)   = evalNoVars a - evalNoVars b
      evalNoVars (TimesVE a b)   = evalNoVars a * evalNoVars b
...
instance Env ListEnv where
  ...
  lookupInEnv s ((v,n):rest)
    | s == v    = Just n
    | otherwise = lookupInEnv s rest
...
evalAll env exprs = map (\e -> varExprEval env e) exprs
...
sumEval env exprs = foldr f (Just 0) exprs
  where f :: VarExpr -> Maybe Int -> Maybe Int
        f expr acc = opMaybe (+) (varExprEval env expr) acc
